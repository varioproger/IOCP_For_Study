...
void MainMngr::Init()
{
	...
	m_soListen = GetListenSocket(9000);
	m_evListen = WSACreateEvent();
	WSAEventSelect(m_soListen, m_evListen, FD_ACCEPT)
...
void MainMngr::Run()
{
	...
	//전용 스레드는 종료 통지, 접속 요청, 레지스트리 변경을 감시하기 위해 3개의 핸들을 배열로 구성한다.
	//여기서 체크가 가능한 이유는 	WSAEventSelect(m_soListen, m_evListen, FD_ACCEPT) 를 해줬기 때문이다.
	...
	while (bContinue)
	{
		...
		);//메시지 및 종료 통지, 접속 요청, 레지스트리 변경 통지를 대기한다.
		...
		if (dwWaitRet == WAIT_OBJECT_0)//종료 통지가 오면 루프를 탈출해 스레드를 종료한다.
			break;

		switch (dwWaitRet)
		{
		case MAIN_WAIT_CNT: bContinue = Handler_Measage(); break;//PostThreadMessage로 인한 메시지가 전달
		case WAIT_OBJECT_0 + 1: bContinue = Handler_Accept(); break;//m_evListen
		case WAIT_OBJECT_0 + 2: bContinue = Handler_Registry(); break;//m_evMonReg
		}
	}
	CloseHandle(m_hIocp);
...
void MainMngr::Start()
{
	...
	//끝나고 나와서 DWORD WINAPI MainMngr::MainMngrProc(PVOID pParam) 실행을 위한 스래드가 생성이 된다.
...
void _tmain()
{
	...
	try
	{
		mngr.Start();// 이부분 부터 시작하면 된다. 1
		//mngr.ConfigMonDir(false, "C:\\Users\\dirnr\\Desktop\\Temp");
